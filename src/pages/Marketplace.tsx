  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      <header className="mb-8 md:mb-12">
        ...
        <p className="text-slate-500 max-w-2xl text-sm md:text-base">Encontre as melhores ferramentas e serviços para escalar sua operação de varejo.</p>
      </header>
      <div className="mb-8 md:mb-12">
        <div className="relative max-w-xl">
          <input type="text" placeholder="O que você está procurando?" className="w-full h-12 md:h-14 pl-12 pr-4 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#0052cc] focus:border-transparent text-sm md:text-base shadow-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {filteredProducts.map((product, index) => (
          <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}><ProductCard product={product} /></motion.div>
        ))}
      </div>
    </div>
  );
